...
void BattlePayDataStoreMgr::LoadDisplayInfos()
{
    ...
    auto result = ShopDatabase.PQuery("SELECT DisplayInfoId, CreatureDisplayInfoID, FileDataID, Flags, Name1, Name2, Name3, Name4 FROM battlepay_display_info");
    if (!result)
        return;
...
void BattlePayDataStoreMgr::LoadDisplayInfoVisuals()
{
    ...
    auto result = ShopDatabase.PQuery("SELECT DisplayInfoId, DisplayId, VisualId, ProductName FROM battlepay_display_info_visuals");
    if (!result)
        return;
...
void BattlePayDataStoreMgr::LoadProductGroups()
{
    ...
    auto result = ShopDatabase.PQuery("SELECT GroupID, Name, IconFileDataID, DisplayType, Ordering FROM battlepay_product_group");
    if (!result)
        return;
...
void BattlePayDataStoreMgr::LoadProduct()
{
    ...
    auto result = ShopDatabase.PQuery("SELECT ProductID, NormalPriceFixedPoint, CurrentPriceFixedPoint, Type, WebsiteType, ChoiceType, Flags, DisplayInfoID, ClassMask, ScriptName FROM battlepay_product");
    if (!result)
        return;
    ...
    result = ShopDatabase.PQuery("SELECT ID, ProductID, ItemID, Quantity, DisplayID, PetResult FROM battlepay_product_item");
    if (!result)
        return;
...
void BattlePayDataStoreMgr::LoadShopEntires()
{
    ...
    auto result = ShopDatabase.PQuery("SELECT EntryID, GroupID, ProductID, Ordering, Flags, BannerType, DisplayInfoID FROM battlepay_shop_entry");
    if (!result)
        return;
...
void BattlePayDataStoreMgr::LoadProductGroupLocales()
{
    ...
    auto result = ShopDatabase.PQuery("SELECT GroupID, Locale, Name FROM battlepay_product_group_locales");
    if (!result)
        return;
...
void BattlePayDataStoreMgr::LoadDisplayInfoLocales()
{
    ...
    auto result = ShopDatabase.PQuery("SELECT Id, Locale, Name1, Name2, Name3, Name4 FROM battlepay_display_info_locales");
    if (!result)
        return;
