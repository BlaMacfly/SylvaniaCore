...
struct boss_azzakel : public BossAI
{
    boss_azzakel(Creature* creature) : BossAI(creature, DATA_AZZAKEL)
    {
        m_Intro = false;
        m_First = true;
    }
...
struct auchindoun_azzakel_mob_controller : public ScriptedAI
{
    ...
    void UpdateAI(uint32 p_p_Diff) override
    {
        ...
        {
            if (Creature* TempAzzakael = me->SummonCreature(NPC_AZZAKEL, *me, TempSummonType::TEMPSUMMON_MANUAL_DESPAWN))
            {
                if (GameObject* l_Portal = me->SummonGameObject(eAuchindounObjects::GameobjectDemonicPortal, *me, QuaternionData(0, 0, 0, 0), 0))
                {
                    ...
                    l_Portal->SetFlag(GAMEOBJECT_FLAGS, GO_FLAG_NOT_SELECTABLE | GO_FLAG_NODESPAWN | GO_FLAG_INTERACT_COND);
...
public:
    ...
    struct auchindoun_azzakel_mob_fel_spark_triggerAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            ...
            me->SetReactState(ReactStates::REACT_PASSIVE);
            me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_REMOVE_CLIENT_CONTROL | UnitFlags::UNIT_FLAG_NON_ATTACKABLE | UnitFlags::UNIT_FLAG_NOT_SELECTABLE | UnitFlags::UNIT_FLAG_IMMUNE_TO_PC);
            ...
            for (uint8 l_I = 0; l_I < 20; ++l_I)
            {
                ...
                if (Creature* l_FelSparkNullAITrigger = me->SummonCreature(eFelSparkCreatures::CreatureFelSparkNullAITrigger, l_X, l_Y, me->GetPositionZ(), me->GetOrientation(), TempSummonType::TEMPSUMMON_TIMED_DESPAWN, 7 * TimeConstants::IN_MILLISECONDS))
                {
                    ...
                    l_FelSparkNullAITrigger->SetFlag(UNIT_FIELD_FLAGS, UnitFlags::UNIT_FLAG_NON_ATTACKABLE | UnitFlags::UNIT_FLAG_NOT_SELECTABLE | UnitFlags::UNIT_FLAG_REMOVE_CLIENT_CONTROL | UnitFlags::UNIT_FLAG_IMMUNE_TO_PC);
    ...
    struct auchindoun_azzakel_mob_fel_poolAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            if (m_First)
            {
                ...
                me->SetDisplayId(InvisibleDisplay);
                me->SetReactState(ReactStates::REACT_PASSIVE);
                me->SetFlag(UNIT_FIELD_FLAGS, UnitFlags::UNIT_FLAG_IMMUNE_TO_NPC | UnitFlags::UNIT_FLAG_IMMUNE_TO_PC | UnitFlags::UNIT_FLAG_NON_ATTACKABLE | UnitFlags::UNIT_FLAG_NOT_SELECTABLE);
            }
            ...
            if (InstanceScript* l_Instance = GetCaster()->GetInstanceScript())
            {
                if (Creature* l_Azzakel = l_Instance->instance->GetCreature(l_Instance->GetGuidData(eAuchindounDatas::DataBossAzzakael)))
                {
                    if (l_Azzakel->IsAIEnabled)
                    {
                        ...
                        l_Azzakel->SetFlag(UNIT_FIELD_FLAGS, UnitFlags::UNIT_FLAG_IMMUNE_TO_PC | UnitFlags::UNIT_FLAG_NON_ATTACKABLE | UnitFlags::UNIT_FLAG_NOT_SELECTABLE);
                        Position pos;
...
{
    RegisterCreatureAI(boss_azzakel);
