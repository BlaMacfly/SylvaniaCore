        ...
        switch (action)
        {
            ...
            case ACTION_2: //Despawn after evade 
            {
                if (instance->GetBossState(DATA_SISTERS_OF_THEMOON) != NOT_STARTED)
                {
                    ...
                    if (auto kasparian = ObjectAccessor::GetCreature(*me, sistersVector[0]))
                ; // frame instance start/end non supporte par notre core
                    sideMoonTimer = 0;
                    berserkTimer = 0;
                    ...
                    {
                        if (auto sister = ObjectAccessor::GetCreature(*me, guid))
                        {
            ...
            {
                if (auto lunaspyre = ObjectAccessor::GetCreature(*me, sistersVector[1]))
                    lunaspyre->AI()->DoAction(ACTION_1); //Lunaspyre set Phase 2

                if (auto yathae = ObjectAccessor::GetCreature(*me, sistersVector[2]))
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (berserkTimer)
        {
            if (berserkTimer <= diff)
            {
                ...
                for (auto const& guid : sistersVector)
                {
                    if (auto boss = ObjectAccessor::GetCreature(*me, guid))
    ...
    void DefaultEvents(uint8 phase)
    {
        ...
        switch (phase)
        {
            ...
            case 2:
                ...
                me->RemoveAurasByType(SPELL_AURA_PERIODIC_DAMAGE); me->RemoveAurasByType(SPELL_AURA_PERIODIC_DAMAGE_PERCENT);
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_TWILIGHT_VOLLEY:
                    if (auto target = SelectTarget(SELECT_TARGET_RANDOM, 0, [this](Unit const* t) { return t->IsPlayer() && t->ToPlayer()->GetRoleForGroup() == ROLE_DAMAGE /* approx ranged-dps */; }))
                    {
                        me->SetFacingToObject(target);
    ...
    void OnRemove(AuraEffect const* /*auraEffect*/, AuraEffectHandleModes /*mode*/)
    {
        ...
        if (GetTarget() && damage)
        {
            if (GetId() == SPELL_EMBRACE_ECLIPSE)
                { int32 bpv = int32(damage); GetTarget()->CastCustomSpell(GetTarget(), SPELL_UMBRA_DETONATION, &bpv, NULL, NULL, true); }
            else if (GetId() == SPELL_EMBRACE_ECLIPSE_FRIEND)
                { int32 bpv = int32(damage); GetTarget()->CastCustomSpell(GetTarget(), SPELL_LUNAR_DETONATION, &bpv, NULL, NULL, true); }
        }
    }
...
struct at_sistersmoon_moon : AreaTriggerAI
{
    ...
    bool IsValidTarget(Unit* caster, Unit* target, AreaTriggerActionMoment actionM) override
    {
        ...
        endPos.SimplePosXYRelocationByAngle(end, dist * (1.0f - at->GetProgressCompat()), angle);
