...
public:
    ...
    struct boss_harjatanAI : BossAI
    {
        ...
        void Reset() override
        {
            ...
            me->GetAreatriggerListInRange(list, 80.0f);
        list.remove_if([](AreaTrigger* at1) { AreaTriggerTemplate const* t1 = at1->GetTemplate(); return !t1 || t1->Id != 13251; });
...
class spell_tos_fixate : public AuraScript
{
    ...
    void Register()
    {
        ...
        AfterEffectRemove += AuraEffectRemoveFn(spell_tos_fixate::OnRemove, SpellEffIndex::EFFECT_0, SPELL_AURA_FIXATE, AuraEffectHandleModes::AURA_EFFECT_HANDLE_REAL);
...
class spell_tos_draw_in : public AuraScript
{
    ...
    void OnTick(AuraEffect const* /*aurEff*/)
    {
        ...
        std::list<AreaTrigger*> list;
        caster->GetAreatriggerListInRange(list, 80.0f);
        list.remove_if([](AreaTrigger* at1) { AreaTriggerTemplate const* t1 = at1->GetTemplate(); return !t1 || t1->Id != 13251; });
        uint8 realCount = 0;
        for (std::list<AreaTrigger*>::const_iterator itr = list.begin(); itr != list.end(); itr++)
        {
            ...
            // adaptation : notre API de creation manuelle d AT differe, on recaste le sort
            // createur (231754) a la meme position pour regenerer la zone avec sa duree fraiche
            caster->CastSpell((*itr)->GetPositionX(), (*itr)->GetPositionY(), (*itr)->GetPositionZ(), 231754, true);
