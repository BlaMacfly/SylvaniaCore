        ...
        switch (action)
        {
            ...
            case ACTION_2: //Despawn after evade 
            {
                if (instance->GetBossState(DATA_THE_DESOLATE_HOST) != NOT_STARTED)
                {
                    ...
                    if (auto host = ObjectAccessor::GetCreature(*me, bossVector[2]))
                ; // frame instance start/end non supporte par notre core
                    RemoveDebuff();
                    ; // SummonCreatureGroupDespawn : groupes d invocation uwow, non porte
                    me->RemoveAllAuras();
            ...
            case NPC_SOUL_QUEEN_DEJAHNA:
            {
                if (++bossDiedCount == 3)
                {
                    if (instance->GetBossState(DATA_THE_DESOLATE_HOST) != DONE)
                    {
                        ...
                        if (auto host = ObjectAccessor::GetCreature(*me, bossVector[2]))
                ; // frame instance start/end non supporte par notre core
                        if (auto engine = ObjectAccessor::GetCreature(*me, bossVector[0]))
                        {
                            if (auto player = killer->GetCharmerOrOwnerPlayerOrPlayerItself())
                                ; // GeneratePersonalLoot : distribution perso uwow, le loot passe par la table classique
                        }
                    }
                    RemoveDebuff();
                    summons.DespawnAll();
        ...
        if (action == ACTION_1)
        {
            me->AddDelayedEvent(4000, [this] () -> void
            {
                if (me)
                {
                    ...
                    Talk(SAY_HOST_AGGRO);
                    DoCast(me, SPELL_SHARED_HEALTH, true);
                    me->SendPlaySpellVisualKit(0, VISUAL_KIT_1, 0);
...
struct npc_tos_reanimated_templar : ScriptedAI
{
    ...
    void IsSummonedBy(Unit* summoner) override
    {
        ...
        {
            if (instance->GetBossState(DATA_THE_DESOLATE_HOST) == IN_PROGRESS)
                me->SendPlaySpellVisualKit(0, VISUAL_KIT_2, 0);
            else
...
class spell_tos_spear_of_anguish : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        targets.remove_if([this](WorldObject* object) -> bool
        {
            ...
            if (object->ToUnit()->IsImmunedToSpell(GetSpellInfo(), nullptr))
                return true;
    ...
    void HandleDummy(SpellEffIndex /*effectIndex*/)
    {
        ...
        auto target = GetHitUnit();
        if (!target || target->IsImmunedToSpell(GetSpellInfo(), nullptr))
            return;
...
class spell_tos_spiritual_barrier_dissonance_visual_aura : public AuraScript
{
    ...
    void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        if (GetTarget())
        {
            GetTarget()->SetFlag(OBJECT_DYNAMIC_FLAGS, UNIT_DYNFLAG_NOT_SELECTABLE_MODEL);

            if (GetTarget()->IsPlayer())
            {
                ; // UpdateCustomField : rafraichissement client custom uwow, non porte
    ...
    void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        if (GetTarget())
        {
            GetTarget()->RemoveFlag(OBJECT_DYNAMIC_FLAGS, UNIT_DYNFLAG_NOT_SELECTABLE_MODEL);
    ...
    void Register() override
    {
        ...
        /* hook EffectUpdate (tick par diff) non supporte par notre AuraScript */;
    }
};
