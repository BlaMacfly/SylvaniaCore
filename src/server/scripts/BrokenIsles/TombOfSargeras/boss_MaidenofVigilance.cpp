...
struct npc_tos_essences : ScriptedAI
{
    npc_tos_essences(Creature* creature) : ScriptedAI(creature)
    {
        ...
        ; // (m_RateUpdateTimer : optimisation de cadence uwow, non portee)
    }
            ...
            for (const auto& itr : players)
            {
                ...
                if (me->GetDistance(who) < 0.22f && !justDoIt)
                {
                    ...
                    if (TempSummon* tempSum = me->ToTempSummon()) // we need it for disabling call SummonedCreatureDespawn
                        ; // (SetSummonerGUID : detachement du summoner, non porte)
    ...
    void CorrectTargets(std::list<WorldObject*>& targets)
    {
        ...
        for (auto itr = targets.begin(); itr != targets.end(); )
        {
            ...
            if (Player* player = target->ToPlayer())
                if (ROLE_TANK == player->GetRoleForGroup() && !spellsForTank.empty()) // tanks have are different buffs
                {
...
class spell_tos_unstable_soul : public AuraScript
{
    ...
    void OnApply(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
    {
        ...
        {
            target->RemoveAppliedAuras(235117, [aura](AuraApplication const* aplication) -> bool
            {
