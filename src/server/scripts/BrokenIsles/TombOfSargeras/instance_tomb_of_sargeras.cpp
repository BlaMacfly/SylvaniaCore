...
{
    //{269164,                    DATA_GOROTH,                DOOR_TYPE_PASSAGE},
...
public:
    ...
    {
        instance_tomb_of_sargeras_InstanceMapScript(Map* map) : InstanceScript(map->ToInstanceMap()) 
        {
        ...
        void OnCreatureCreate(Creature* creature) override
        {
            ...
            switch (creature->GetEntry())
            {
                ...
                case 121023:
                    ...
                    {
                        if (!creature->HasAura(35356) && !false /* flag3 feign death non porte */)
                        {
                            creature->AddDelayedEvent(1000, [creature] () -> void {creature->SetUInt32Value(UNIT_NPC_EMOTESTATE, EMOTE_ONESHOT_ATTACK1H);});
                ...
                case NPC_MAEIV:
                case NPC_ILLIDAN:
                    creature->SetUInt32Value(UNIT_NPC_EMOTESTATE, EMOTE_ONESHOT_ATTACK1H);
                    break;
                case NPC_VELEN:
            ...
            switch (go->GetEntry())
            {
                case GO_ELEVATOR:
                    if (GetBossState(DATA_MAIDEN_OF_VIGILANCE) != DONE)
                        // (SetPhaseMask 2 : phasing legacy uwow, non porte)
                    break;
                case 267700:
                case 267701:
                case 267702:
        ...
        bool SetBossState(uint32 type, EncounterState state) override
        {
            ...
            if (type != DATA_MAIDEN_OF_VIGILANCE && type != DATA_FALLEN_AVATAR && type != DATA_KILJAEDEN)
                    if (GameObject* go = instance->GetGameObject(GetGuidData(269164)))
                        HandleGameObject(go->GetGUID(), CheckRequiredBosses(DATA_MAIDEN_OF_VIGILANCE), go);
                    ...
                    switch (type)
                    {
                        ...
                        case DATA_MAIDEN_OF_VIGILANCE:
                            if (auto go = instance->GetGameObject(GetGuidData(GO_ELEVATOR)))
                                // (SetPhaseMask 1 : phasing legacy uwow, non porte)
                            break;
                        default:
                            break;
                    }
        ...
        WorldLocation* GetClosestGraveYard(float x, float y, float z) /* signature uwow, la table graveyard_zone fait foi */
        {
            ...
            if (CheckRequiredBosses(DATA_MAIDEN_OF_VIGILANCE))
            ...
            for (auto gravYard : availableGravs)
            {
                ...
                Position pos1(temp->Loc.X, temp->Loc.Y, temp->Loc.Z);
                Position pos2(saved->Loc.X, saved->Loc.Y, saved->Loc.Z);
