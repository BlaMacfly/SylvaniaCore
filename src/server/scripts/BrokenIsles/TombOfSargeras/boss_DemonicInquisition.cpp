...
public:
    ...
    struct boss_demonic_inquisitionAI : BossAI
    {
        ...
        void DamageTaken(Unit* attacker, uint32& damage) override
        {
            ...
            // approximation : notre DamageTaken n expose pas le type de degats,
            // le renvoi d epines se declenche donc sur tout degat (l original excluait DoT/split)
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_SUFFOCATING_DARK:
                    {
                        ...
                        {
                            return ref->getTarget()->ToPlayer()->GetRoleForGroup() == ROLE_TANK; // approximation : on ecarte seulement les tanks
                        });
...
public:
    ...
    class spell_tos_torment_AuraScript : public AuraScript
    {
        ...
        void Register() override
        {
            ...
            AfterEffectRemove += AuraEffectRemoveFn(spell_tos_torment_AuraScript::OnRemove, SpellEffIndex::EFFECT_1, SPELL_AURA_PERIODIC_DUMMY, AuraEffectHandleModes::AURA_EFFECT_HANDLE_REAL);
        }
    };
