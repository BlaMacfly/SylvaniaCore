...
struct npc_goroth_infernal_spike : public ScriptedAI
{
    ...
    void DoAction(int32 const action) override
    {
        ...
        ; // (PlaySpellVisual positionnel : cosmetique uwow, non porte)
...
class spell_tos_goroth_crashing_comet : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        {
            if (auto spike = ObjectAccessor::GetCreature(*GetCaster(), infernalSpike->GetGUID()))
    ...
    void HandleScript(SpellEffIndex /*effectIndex*/)
    {
        ...
        if (InstanceScript* instance = GetCaster()->GetInstanceScript())
            if (Creature* goroth = instance->instance->GetCreature(instance->GetGuidData(DATA_GOROTH)))
                goroth->CastSpell(GetHitDest()->GetPositionX(), GetHitDest()->GetPositionY(), GetHitDest()->GetPositionZ(), SPELL_FEL_ERUPTION_AT, true);
    }
...
class spell_tos_goroth_rain_of_brimstone : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        GetCaster()->CastSpell(GetExplTargetDest()->GetPositionX(), GetExplTargetDest()->GetPositionY(), GetExplTargetDest()->GetPositionZ(), SPELL_RAIN_OF_BRIMSTONE_SPIKE_DESTROY, true);

        if (targets.empty())
            GetCaster()->CastSpell(GetExplTargetDest()->GetPositionX(), GetExplTargetDest()->GetPositionY(), GetExplTargetDest()->GetPositionZ(), SPELL_RAIN_OF_BRIMSTONE_SUMMON, true);
        else
...
struct at_goroth_infernal_spike_los_blocker : AreaTriggerAI
{
    ...
    bool IsValidTarget(Unit* caster, Unit* target, AreaTriggerActionMoment actionM) override
    {
        if (caster && target && target->IsPlayer())
        {
            ...
            {
                if (auto goroth = ObjectAccessor::GetCreature(*target, instance->GetGuidData(DATA_GOROTH)))
                {
    ...
    void OnDestinationReached() override
    {
        ...
        if (auto spellInfo = sSpellMgr->GetSpellInfo(SPELL_SHATTERING_NOVA))
        {
            if (auto effInfo = spellInfo->GetEffect(EFFECT_0, caster->GetMap()->GetDifficultyID()))
            {
                ...
                ; // (SetCaster : le caster est fourni au CastSpell, inutile chez nous)
