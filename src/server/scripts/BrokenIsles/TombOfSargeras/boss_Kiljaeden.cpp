...
struct boss_tos_kiljaeden : BossAI
{
    ...
    void JustDied(Unit* /*killer*/) override
    {
        ...
        {
            if ((me->getThreatManager().getThreat(player) > 0.0f))
    ...
    void DamageTaken(Unit* /*attacker*/, uint32& damage) override
    {
        if (me->HealthBelowPct(healthPct))
        {
            if (healthPct == 81)
            {
                ...
                me->StopAttack(true);
                SetFlyMode(true);
                me->SetByteValue(UNIT_FIELD_BYTES_1, 3, 3);
                me->SetSpeed(MOVE_FLIGHT, 3.0f);
    ...
    void Armageddon()
    {
        ...
        me->GetPosition().GenerateNonDuplicatePoints(randPosList, centrPos, armageddonCount, 15.0f, 38.0f, 8.0f);
        uint8 count = 0;
...
struct npc_tos_armageddon_stalker : public ScriptedAI
{
    ...
    void SpellHit(Unit* caster, SpellInfo const* spell) override
    {
        ...
        if (spell->Id == SPELL_ARMAGEDDON_RAIN_SELF)
        {
            ...
            if (auto at = me->GetAreaTrigger(SPELL_ARMAGEDDON_RAIN_AT))
            {
        ...
        if (spell->Id == SPELL_ARMAGEDDON_HAIL_SELF)
        {
            ...
            if (auto at = me->GetAreaTrigger(SPELL_ARMAGEDDON_HAIL_AT))
            {
                if (at->GetInsideUnits().empty())
                    owner->CastSpell(me, SPELL_ARMAGEDDON_BLAST, true);
            }
...
struct npc_tos_stage4_illidan_stormrage : public ScriptedAI
{
    ...
    void IsSummonedBy(Unit* summoner) override
    {
        me->PlayOneShotAnimKitId(ILLIDAN_ANIM_1);
...
struct npc_tos_demonic_obelisk : public ScriptedAI
{
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (animTimer)
        {
            ...
            {
                animTimer = 0;
                me->PlayOneShotAnimKitId(OBELISK_ANIM_1);
            }
            else
                animTimer -= diff;
        }
        if (obeliskDmgTimer)
        {
            if (obeliskDmgTimer <= diff)
            {
                ...
                me->PlayOneShotAnimKitId(OBELISK_ANIM_2);
        ...
        if (riftSumTimer)
        {
            if (riftSumTimer <= diff)
            {
                ...
                if (auto owner = me->GetAnyOwner())
                {
                    ...
                    owner->GetAreaTriggerListWithSpellIDInRange(listAT, SPELL_FLAMING_DETONATION, 300.0f);
                    (void)listAT; // CastAction : declenchement manuel d action AT uwow, non porte
                }
            }
            else
                riftSumTimer -= diff;
...
class spell_tos_darkness_thousand_souls : public SpellScript
{
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        if (auto caster = GetCaster())
        {
            ...
            {
                if (auto rift = ObjectAccessor::GetCreature(*caster, RiftGuid))
...
class spell_tos_flaming_orb_fixate : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        targets.sort([caster = GetCaster()](WorldObject* a, WorldObject* b) { return caster->GetDistance(a) < caster->GetDistance(b); });
...
struct at_tos_flaming_orb : AreaTriggerAI
{
    ...
    bool IsValidTarget(Unit* caster, Unit* target, AreaTriggerActionMoment actionM) override
    {
        ...
        {
            int32 bp0 = int32(scale);
            caster->CastCustomSpell(caster, SPELL_FLAMING_DETONATION, &bp0, nullptr, nullptr, true, nullptr, nullptr, instance->GetGuidData(DATA_KILJAEDEN));
            at->Remove();
        }
...
struct at_tos_flaming_detonation : AreaTriggerAI
{
    ...
    {
        if (SpellInfo const* si = sSpellMgr->GetSpellInfo(at->GetSpellId()))
            if (SpellEffectInfo const* ei = si->GetEffect(EFFECT_0))
                createScale = CalculatePct(30.0f, ei->BasePoints * 100.0f);
