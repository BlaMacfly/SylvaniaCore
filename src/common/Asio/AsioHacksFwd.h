...
// In Boost 1.70+ basic_deadline_timer gained an extra Executor template parameter,
// so it can no longer be forward-declared by hand. Include the real header instead.
...
#if BOOST_VERSION >= 107000
        // deadline_timer is provided by the included header (its template signature
        // changed in Boost 1.70+). tcp_resolver stays a TrinityCore convenience typedef.
        namespace ip
        {
